...
def store_v4i32_shmem(vec: Any, addr_i64: Any) -> None:
    """Store 128-bit (4 × i32) via AMDGPU inline asm ``flat_store_dwordx4``.

    Emits a **single** ``flat_store_dwordx4 sc0 sc1`` instruction via LLVM
    inline assembly:

    * ``sc0`` — system-coherent (L2 bypass, cross-GPU visible via XGMI)
    * ``sc1`` — globally-coherent (NIC-level, matches mori WarpCopy semantics)

    This is the inline-assembly equivalent of mori's ``WarpCopy store<16>``
    on ``hipDeviceMallocUncached`` memory.  One instruction vs the 4 ×
    ``flat_store_dword sc0`` previously generated by :func:`store_i32_shmem` × 4.

    VGPR constraint mapping (``"v,v"``):

    * ``$0``  ``ptr addrspace(0)``  →  VGPR pair   ``[va+1:va]``  (64-bit flat addr)
    * ``$1``  ``vector<4xi32>``     →  VGPR quad   ``[vd+3:vd]``  (128-bit data)

    Args:
        vec:      ``vector<4xi32>`` value (from :func:`load_v4i32`).
        addr_i64: i64 destination address (flat, 16-byte aligned).
    """
    vec_val  = _unwrap(vec)
    addr_val = _unwrap(addr_i64)

    # Convert i64 → flat ptr so LLVM places the address in a VGPR pair
    ptr_flat = llvm.PointerType.get(address_space=0)
    gptr = llvm.IntToPtrOp(ptr_flat, addr_val).result

    # Emit the single 128-bit store instruction via ir.Operation.create.
    # We bypass llvm.InlineAsmOp() because its Python binding requires a
    # non-empty result type list even for void asm; Operation.create lets us
    # supply zero results and all required LLVM-dialect attributes directly.
    _i64t = ir.IntegerType.get_signless(64)
    ir.Operation.create(
        "llvm.inline_asm",
        results=[],
        operands=[gptr, vec_val],
        attributes={
            # AT&T asm dialect (0 = att, 1 = intel)
            "asm_dialect":      ir.IntegerAttr.get(_i64t, 0),
            # flat_store_dwordx4 addr=$0, data=$1 [offset omitted→0], sc0 sc1
            # Note: offset must be omitted (defaults to 0) or written as
            # "offset:0", NOT as a bare "0" — the AMDGPU assembler rejects
            # the comma-separated positional format ",0".
            "asm_string":       ir.StringAttr.get(
                                    "flat_store_dwordx4 $0, $1 sc0 sc1"),
            # v,v — both operands in VGPRs (addr=VGPR_64, data=VGPR_128)
            "constraints":      ir.StringAttr.get("v,v"),
            # Mark as side-effecting so the compiler cannot reorder/elide it
            "has_side_effects": ir.UnitAttr.get(),
            "tail_call_kind":   ir.Attribute.parse("#llvm.tailcallkind<none>"),
        },
    )
